...
class AdaGrad(BaseOptimizer):
    ...
    def optimize(self):
        ...
        progress_bar = tqdm(range(1, self.max_iter+1), desc=f'Adagrad {num_iter}',) if self.verbose else range(1, self.max_iter+1)
        ...
        return {'method_name': 'Adagrad' if not self.f_cons else 'Adagrad with Penalty',
                'x0':self.x0,
                'xopt': x,
                'fmin': self.f_obj(x),
                'num_iter': _,
