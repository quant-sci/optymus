...
def mma(m: int, n: int, iter: int, xval: np.ndarray, xmin: np.ndarray, xmax: np.ndarray,
    ...
    Args:
        ...
        iter (int): Current iteration number (1 for the first call to mma).
    ...
    xmma, ymma, zmma, lam, xsi, eta, mu, zet, s = sub_solver(
        m, n, epsimin, low, upp, alfa, beta, p0, q0, P, Q, a0, a, b, c, d)
    
    # Return values
    return xmma, ymma, zmma, lam, xsi, eta, mu, zet, s, low, upp

def sub_solver(m: int, n: int, epsimin: float, low: np.ndarray, upp: np.ndarray, alfa: np.ndarray, 
...
def check_kkt(m: int, n: int, x: np.ndarray, y: np.ndarray, z: float, lam: np.ndarray, xsi: np.ndarray,
