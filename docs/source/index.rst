...
`optymus` is a Python library for solving optimization problems in engineering and scientific computing. Built on `JAX <https://jax.readthedocs.io/en/latest/index.html>`_ for automatic differentiation, it provides efficient gradient computation and GPU acceleration. The library supports continuous optimization, topology optimization, and combinatorial optimization (vehicle routing).
...
See the :doc:`examples/index` for detailed tutorials.
